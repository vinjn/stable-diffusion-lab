...
python -m accelerate.commands.launch ${launchArgs[@]} --num_cpu_threads_per_process=8 "$SCRIPT_DIR/sd-scripts/train_network.py" \
  --enable_bucket \
  --pretrained_model_name_or_path=$pretrained_model \
  --train_data_dir=$train_data_dir \
  --output_dir="$SCRIPT_DIR/_output" \
  --logging_dir="$SCRIPT_DIR/_log" \
